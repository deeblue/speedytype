...
## Local Gemma comparison (2026-07-12)

A fresh isolated 38-call polishing-only benchmark compared Gemini Flash-Lite with local Ollama Gemma 12B and 26B while STT remained `whisper-1`. Both local candidates were unloaded before each cold trial; no `ollama ps` snapshot contained both models. All calls succeeded. Gemini passed 12/12 warm quality checks at a 0.636-second mean; 26B passed 12/12 at 12.883 seconds and had a 37.355-second cold call; 12B passed 9/12 and failed all three repeated-number trials. The measured evidence supports the inference to retain Gemini as default and consider 26B only as an optional privacy/offline trade-off. No provider, model, or STT default changed. See [LOCAL_LLM_EXPERIMENT_REPORT.md](LOCAL_LLM_EXPERIMENT_REPORT.md).
