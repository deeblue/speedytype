...
A 38-call polishing-only benchmark compared the current Gemini Flash-Lite setting with local Ollama Gemma 12B and 26B while STT remained `whisper-1` throughout. All provider calls succeeded. Gemini passed 12/12 quality checks with a 0.590-second warm mean; 26B passed 13/13 but had an 11.773-second warm mean and 40.869-second cold call; 12B passed only 10/13 and failed all three repeated-number trials. The evidence supports retaining Gemini as the default and exposing 26B local mode only as an optional privacy/offline trade-off. See [LOCAL_LLM_EXPERIMENT_REPORT.md](LOCAL_LLM_EXPERIMENT_REPORT.md) for raw-record references, percentile method, diagnostics, cost assumptions, and limitations.
